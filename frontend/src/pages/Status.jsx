    ...
    try {
      ...
      if (!Array.isArray(res.data) || res.data.length < 1) {
        toast.error("Gagal Mendapatkan Data", {
          description: "Data rekor tidak ditemukan.",
        });
      } else {
          ...
          <table className="min-w-full bg-white border border-gray-300 shadow-md rounded-lg">
            <thead>
              <tr className="bg-red-600 text-white">
                ...
                <th className="py-3 px-4 text-center">Alasan</th>
            ...
            <tbody>
              {data.map((item, index) => (
                ...
                >
                  ...
                  <td className="border-b px-15 border-gray-200 py-3">
                  {new Date(item.tgl_tidakhadir).toLocaleDateString()}
                  </td>
                  <td className="border-b border-gray-200 py-3 px-5">
                    <span
                      className={`inline-block px-5 py-1 rounded-full text-xs font-semibold ${getStatusColor(
